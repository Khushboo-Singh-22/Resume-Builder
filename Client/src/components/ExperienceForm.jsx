...
const ExperienceForm = ({ data, onChange }) => {
  ...
  const generateDescription = async (index) =>{
    ...
    const prompt = `enhance this job description ${experince.description} for the position of ${experince.position} at ${experince.company}.`

    try {
      const {data: responseData} = await API.post('/api/ai/enhance-job-desc',{userContent: prompt}, {headers: {Authorization: `Bearer ${token}`}})
      ...
      <div>
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm text-gray-500'>Add your resume job experience</p>
          </div>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
                    {genratingIndex === index ?  (
                      <Loader2 className='w-3 h3 animate-spin'/>
                    ):(
